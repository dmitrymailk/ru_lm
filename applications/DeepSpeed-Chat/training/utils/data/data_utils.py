...
def prepare_dataset_v7(
    ...
    formated_prompt = prompt_func(example)
    formated_prompt = convert_prompt_saiga_version_v2(prompt=formated_prompt)
    prompt = encode_prompt_v2(
        prompt=formated_prompt,
        tokenizer=tokenizer,
        start_token_id=2,
        end_token_id=3,
...
PREPARE_FUNC = {
    ...
    "v6": prepare_dataset_v6,
    "v7": prepare_dataset_v7,
}
...
class DataCollatorRLHF:
    ...
    def __call__(self, data):
        ...
        if pad_length > 0:
            batch["prompt"] = F.pad(
                prompt, pad=(pad_length, 0), mode="constant", value=pad_token_id
            )
            batch["prompt_att_mask"] = F.pad(
                prompt_mask, pad=(pad_length, 0), mode="constant", value=0
            )
        else:
            batch["prompt"] = prompt
